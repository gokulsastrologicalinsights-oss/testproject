    ...
    <section className="w-full py-16 bg-sandal-100/40 dark:bg-zinc-900/20 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col gap-12">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-zinc-650 dark:text-zinc-400 max-w-xl font-light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {stories.map((story, i) => (
            <div key={i} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-md border border-sandal-200 dark:border-zinc-800 text-left flex flex-col gap-4">
              <div className="w-full h-48 rounded-xl bg-gradient-to-tr from-rose-100 to-amber-100 dark:from-zinc-850 dark:to-zinc-800 flex items-center justify-center relative overflow-hidden">
                <Heart className="h-16 w-16 text-maroon-500/30 animate-pulse" />
                <span className="absolute bottom-4 left-4 bg-white/95 dark:bg-zinc-900/95 px-3 py-1 rounded-full text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                  {story.date}
                </span>
              </div>
              <div>
                <h4 className="text-lg font-serif font-bold text-zinc-805 dark:text-zinc-200">
                  {story.name}
                </h4>
                <p className="text-xs text-gold-600 dark:text-gold-400 font-semibold mb-2">{story.compatibility}</p>
